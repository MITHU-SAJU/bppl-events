...
import { getDebitNotes, createDebitNote, deleteDebitNote, getInvoices } from "../services/api";
import Navbar from "../components/Navbar";
import Sidebar from "../components/Sidebar";
import beatslogo from "../assets/beats-logo.jpg";

const DEFAULT_TERMS = `a) All debit adjustments are subject to verification and approval.
b) Approved debit values will be automatically added to the balance due of the designated Tax Invoice.
c) Any query regarding this debit note must be submitted in writing within 24 hours of issuance.`;
...
function AdminDebitNotes() {
  ...
  const loadDebitNotes = async () => {
    ...
    try {
      ...
      setDebitNotes(res.data.reverse());
    } catch (e) { console.error("Error loading debit notes:", e); }
  ...
  const generateDebitNoteNumber = () => {
    ...
    return `DN-${year}-${randomSuffix}`;
  };

  const handleOpenAddForm = () => {
    setDebitNoteForm({
      invoiceNumber: generateDebitNoteNumber(),
      invoiceDate: new Date().toISOString().split("T")[0],
      eventDate: "",
      clientName: "",
      companyName: "",
  ...
  const handleItemChange = (index, field, value) => {
    const updatedItems = [...debitNoteForm.items];
    ...
    const calc = recalculateAmounts(updatedItems, debitNoteForm.cgstRate, debitNoteForm.sgstRate, debitNoteForm.paidAmount);
    setDebitNoteForm({ ...debitNoteForm, items: updatedItems, ...calc });
  };

  const handleAddItemRow = () => {
    const updatedItems = [...debitNoteForm.items, { description: "", quantity: 1, rate: 0, amount: 0 }];
    const calc = recalculateAmounts(updatedItems, debitNoteForm.cgstRate, debitNoteForm.sgstRate, debitNoteForm.paidAmount);
    setDebitNoteForm({ ...debitNoteForm, items: updatedItems, ...calc });
  };

  const handleRemoveItemRow = (index) => {
    if (debitNoteForm.items.length === 1) return;
    const updatedItems = debitNoteForm.items.filter((_, idx) => idx !== index);
    const calc = recalculateAmounts(updatedItems, debitNoteForm.cgstRate, debitNoteForm.sgstRate, debitNoteForm.paidAmount);
    setDebitNoteForm({ ...debitNoteForm, items: updatedItems, ...calc });
  };

  const handleFinancialFieldChange = (field, value) => {
    const updatedForm = { ...debitNoteForm, [field]: parseFloat(value) || 0 };
    const calc = recalculateAmounts(updatedForm.items, updatedForm.cgstRate, updatedForm.sgstRate, updatedForm.paidAmount);
    setDebitNoteForm({ ...updatedForm, ...calc });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setErrorMsg("");
    setSuccessMsg("");
    if (!debitNoteForm.invoiceId) { setErrorMsg("Please select a parent invoice."); return; }
    if (!debitNoteForm.invoiceNumber.trim()) { setErrorMsg("Debit Note number is required."); return; }
    try {
      await createDebitNote(debitNoteForm);
      setSuccessMsg("Debit Note created successfully and parent Invoice balance adjusted!");
      setTimeout(() => { setViewState("list"); loadDebitNotes(); }, 1000);
    } catch (err) {
      setErrorMsg(err.response?.data?.detail || "An error occurred saving the debit note.");
    }
  };

  const handleDeleteDebitNote = async (id) => {
    if (window.confirm("Delete this Debit Note? This will restore the parent Invoice balance.")) {
      try {
        await deleteDebitNote(id);
        loadDebitNotes();
      } catch (err) { alert(err.response?.data?.detail || "Failed to delete Debit Note"); }
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    const runHtml2Pdf = () => {
      const opt = {
        margin: 0,
        filename: `${debitNoteForm.invoiceNumber}.pdf`,
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h2 className="fw-extrabold m-0" style={{ letterSpacing: "-1px" }}>
                {viewState === "list" && "Debit Note Management"}
                {viewState === "add" && "Create Debit Note"}
                {viewState === "preview" && "Debit Note Print Preview"}
              </h2>
              <p className="text-muted small m-0 mt-1">
                {viewState === "list" && "Create debit notes to adjust Tax Invoice balances"}
          ...
          {viewState === "list" ? (
                    ...
                    <tbody>
                      {debitNotes.map((note) => (
                        <tr key={note._id}>
                          ...
                          <td><span className="fw-extrabold text-success font-monospace">₹{formatINR(note.totalAmount)}</span></td>
                          ...
                          <td className="text-center">
                            ...
                            <button className="btn btn-sm btn-danger px-2" onClick={() => handleDeleteDebitNote(note._id)}>Delete</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="card shadow-sm border-0 fade-in">
              <div className="card-header card-gradient-blue p-4 border-0">
                <h5 className="mb-0 fw-bold">Debit Note Form</h5>
              </div>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="row g-3 mb-4">
                    <div className="col-12 col-md-4">
                      ...
                      <select className="form-select" required value={debitNoteForm.invoiceId} onChange={(e) => handleInvoiceSelect(e.target.value)}>
                    ...
                    <div className="col-12 col-md-4">
                      <label className="form-label font-monospace text-secondary small fw-bold">Debit Note Number</label>
                      <input type="text" className="form-control font-monospace fw-bold" required value={debitNoteForm.invoiceNumber} onChange={(e) => setDebitNoteForm({ ...debitNoteForm, invoiceNumber: e.target.value })} />
                    </div>
                    <div className="col-12 col-md-4">
                      <label className="form-label font-monospace text-secondary small fw-bold">Debit Note Date</label>
                      <input type="date" className="form-control" required value={debitNoteForm.invoiceDate} onChange={(e) => setDebitNoteForm({ ...debitNoteForm, invoiceDate: e.target.value })} />
                    </div>
                  </div>

                  {debitNoteForm.invoiceId && (
                    <>
                      <h5 className="fw-bold mb-3">Adjust Line Items (Amounts to debit)</h5>
                      <div className="table-responsive border-0 mb-4">
                        <table className="table align-middle table-hover mb-0">
                          ...
                          <tbody>
                            {debitNoteForm.items.map((item, idx) => (
                              <tr key={idx}>
                                ...
                                <td className="text-center">
                                  <button type="button" className="btn btn-sm btn-outline-danger" disabled={debitNoteForm.items.length === 1} onClick={() => handleRemoveItemRow(idx)}>🗑️</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <div className="col-12 col-md-6">
                          <div className="row g-3">
                            <div className="col-6">
                              ...
                              <input type="number" className="form-control font-monospace" value={debitNoteForm.cgstRate} onChange={(e) => handleFinancialFieldChange("cgstRate", e.target.value)} />
                            </div>
                            <div className="col-6">
                              <label className="form-label font-monospace text-secondary small fw-bold">SGST Rate (%)</label>
                              <input type="number" className="form-control font-monospace" value={debitNoteForm.sgstRate} onChange={(e) => handleFinancialFieldChange("sgstRate", e.target.value)} />
                            </div>
                          </div>
                        </div>
                        <div className="col-12 col-md-6">
                          <div className="card bg-light border-0 p-3 shadow-sm text-end">
                            <div className="d-flex justify-content-between mb-2"><span>Subtotal Debit:</span><span className="fw-bold font-monospace">₹{formatINR(debitNoteForm.subtotal)}</span></div>
                            <div className="d-flex justify-content-between mb-2"><span>CGST ({debitNoteForm.cgstRate}%):</span><span className="fw-bold font-monospace">₹{formatINR(debitNoteForm.cgstAmount)}</span></div>
                            <div className="d-flex justify-content-between mb-2"><span>SGST ({debitNoteForm.sgstRate}%):</span><span className="fw-bold font-monospace">₹{formatINR(debitNoteForm.sgstAmount)}</span></div>
                            <hr className="my-2" />
                            <div className="d-flex justify-content-between mb-2"><span className="fw-bold text-success">TOTAL DEBIT VALUE:</span><span className="fw-extrabold font-monospace text-success fs-5">₹{formatINR(debitNoteForm.totalAmount)}</span></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="row g-3">
                        <div className="col-12 col-md-6">
                          ...
                          <input type="text" className="form-control fw-bold font-monospace" value={debitNoteForm.totalAmountWords} onChange={(e) => setDebitNoteForm({ ...debitNoteForm, totalAmountWords: e.target.value })} />
                        </div>
                        <div className="col-12 col-md-6">
                          <label className="form-label font-monospace text-secondary small fw-bold">Reason for debit (Notes)</label>
                          <textarea className="form-control" rows="2" value={debitNoteForm.notes} onChange={(e) => setDebitNoteForm({ ...debitNoteForm, notes: e.target.value })} />
                        </div>
                        <div className="col-12">
                          <label className="form-label font-monospace text-secondary small fw-bold">Terms &amp; Conditions</label>
                          <textarea className="form-control font-monospace small" rows="3" value={debitNoteForm.terms} onChange={(e) => setDebitNoteForm({ ...debitNoteForm, terms: e.target.value })} />
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  <div className="d-flex justify-content-end gap-3 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" className="btn btn-success px-5" disabled={!debitNoteForm.invoiceId}>Issue Debit Note</button>
                  </div>
                </form>
              </div>
            </div>
            ...
            <div className="d-flex flex-column align-items-center bg-dark bg-opacity-10 py-4 overflow-auto rounded-3">
              <div className="invoice-paper-preview invoice-page mb-5 shadow" id="invoice-page-1">
                <div className="text-center mb-4">
                  <h1 className="fw-extrabold text-uppercase m-0" style={{ fontSize: "2.1rem", letterSpacing: "3px" }}>Debit Note</h1>
                </div>
                ...
                <div className="row mb-4">
                  <div className="col-7">
                    ...
                    <div style={{ fontSize: "0.85rem", lineHeight: "1.4" }}>
                      {debitNoteForm.companyName ? (
                        <>
                          <strong className="d-block mb-0" style={{ fontSize: "1rem" }}>{debitNoteForm.companyName}</strong>
                          {debitNoteForm.clientName && <span className="text-secondary small d-block mb-1">Attn: {debitNoteForm.clientName}</span>}
                        </>
                      ) : (
                        <strong className="d-block mb-1">{debitNoteForm.clientName}</strong>
                      )}
                      <div style={{ whiteSpace: "pre-wrap", color: "#333" }}>{debitNoteForm.clientAddress}</div>
                      {debitNoteForm.clientGst && <div className="mt-1"><strong>GSTIN:</strong> {debitNoteForm.clientGst}</div>}
                    </div>
                  </div>
                  <div className="col-5 text-end">
                    <div className="d-flex flex-column gap-2" style={{ fontSize: "0.85rem" }}>
                      <div className="d-flex justify-content-end align-items-center">
                        ...
                        <strong style={{ fontSize: "1.2rem", fontFamily: "monospace" }}>{debitNoteForm.invoiceNumber}</strong>
                      </div>
                      <div><span className="text-secondary me-2">Date :</span><span>{formatDateShort(debitNoteForm.invoiceDate)}</span></div>
                      <div><span className="text-secondary me-2">Parent Invoice :</span><strong className="font-monospace text-primary">{debitNoteForm.parentInvoiceNumber}</strong></div>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="table mb-0 align-middle">
                    <thead>
                      <tr>
                        ...
                        <th className="pdf-table-header" style={{ width: "54%" }}>Debit Adjustment Details</th>
                ...
                <div className="row g-3" style={{ fontSize: "0.85rem" }}>
                  <div className="col-7">
                    ...
                    <p className="text-secondary">{debitNoteForm.notes || "No extra remarks provided."}</p>
                  </div>
                  <div className="col-5">
                    <table className="w-100 align-middle" style={{ borderCollapse: "collapse", lineHeight: "1.6" }}>
                      <tbody>
                        <tr><td>Subtotal Debit</td><td className="text-end font-monospace">₹ {formatINR(debitNoteForm.subtotal)}</td></tr>
                        <tr><td>CGST ({debitNoteForm.cgstRate}%)</td><td className="text-end font-monospace">₹ {formatINR(debitNoteForm.cgstAmount)}</td></tr>
                        <tr style={{ borderBottom: "1px solid #aaa" }}><td className="pb-1">SGST ({debitNoteForm.sgstRate}%)</td><td className="text-end font-monospace pb-1">₹ {formatINR(debitNoteForm.sgstAmount)}</td></tr>
                        <tr className="fw-extrabold" style={{ fontSize: "0.95rem" }}><td className="pt-2">Total Debit Adjusted</td><td className="text-end font-monospace pt-2">₹ {formatINR(debitNoteForm.totalAmount)}</td></tr>
                      </tbody>
                    </table>
                  </div>
                </div>
                <div className="mt-4" style={{ fontSize: "0.85rem" }}>
                  <strong>Total Debit Adjusted (in words) :</strong><br />
                  <span className="fw-bold mt-1 d-block text-secondary">{debitNoteForm.totalAmountWords}</span>
                </div>
              </div>
              ...
              <div className="invoice-paper-preview invoice-page shadow" id="invoice-page-2">
                <div style={{ minHeight: "80%" }}>
                  <div className="mb-4" style={{ fontSize: "0.75rem", color: "#444", textAlign: "justify" }}>
                    ...
                    <div style={{ whiteSpace: "pre-wrap", lineHeight: "1.3" }}>{debitNoteForm.terms}</div>
                  </div>
                  <div className="border border-secondary border-opacity-25 p-3 rounded-1 mb-5" style={{ fontSize: "0.85rem", color: "#333", backgroundColor: "#f9f9f9" }}>
                    By signing this document, the customer agrees to the debit adjustments listed.
                  </div>
