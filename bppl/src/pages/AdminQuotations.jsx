...
function AdminQuotations() {
  ...
  const loadQuotations = async () => {
    ...
    try {
      ...
      setQuotations(res.data.reverse());
    } catch (e) { console.error("Error loading quotations:", e); }
  ...
  const handleItemChange = (index, field, value) => {
    ...
    const calc = recalculateAmounts(updatedItems, quotationForm.cgstRate, quotationForm.sgstRate, quotationForm.paidAmount);
    setQuotationForm({ ...quotationForm, items: updatedItems, ...calc });
  };

  const handleAddItemRow = () => {
    const updatedItems = [...quotationForm.items, { description: "", quantity: 1, rate: 0, amount: 0 }];
    const calc = recalculateAmounts(updatedItems, quotationForm.cgstRate, quotationForm.sgstRate, quotationForm.paidAmount);
    setQuotationForm({ ...quotationForm, items: updatedItems, ...calc });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setErrorMsg("");
    setSuccessMsg("");
    if (!quotationForm.invoiceNumber.trim()) { setErrorMsg("Quotation number is required."); return; }
    try {
      if (viewState === "add") {
        await createQuotation(quotationForm);
        setSuccessMsg("Quotation created successfully!");
      } else {
        await updateQuotation(currentId, quotationForm);
        setSuccessMsg("Quotation updated successfully!");
      }
      setTimeout(() => { setViewState("list"); loadQuotations(); }, 1000);
    } catch (err) {
      setErrorMsg(err.response?.data?.detail || "An error occurred saving the quotation.");
    }
  };

  const handleConvertToProformaInvoice = async (quot) => {
    ...
    try {
      const proformaData = {
        ...
        status: "Draft",
        sourceQuotationId: quot._id
      };
      delete proformaData._id;
  ...
  const handleConvertToRealInvoice = async (quot) => {
    if (!window.confirm("Convert this Quotation directly to a Tax Invoice?")) return;
    try {
      const invoiceData = {
        ...quot,
  ...
  const handleDeleteQuotation = async (id) => {
    if (window.confirm("Are you sure you want to delete this quotation?")) {
      try {
        ...
        loadQuotations();
      } catch (err) { alert(err.response?.data?.detail || "Failed to delete quotation"); }
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    const runHtml2Pdf = () => {
      const opt = {
        margin: 0,
        filename: `${quotationForm.invoiceNumber}.pdf`,
  ...
  const getStatusBadgeClass = (status) => {
    switch (status) {
      case "Accepted": return "bg-success text-white px-3 py-2 rounded-3 small fw-bold font-monospace";
      case "Rejected": case "Expired": return "bg-danger text-white px-3 py-2 rounded-3 small fw-bold font-monospace";
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h2 className="fw-extrabold m-0" style={{ letterSpacing: "-1px" }}>
                {viewState === "list" && "Quotation Management"}
                {viewState === "add" && "Create Quotation"}
                {viewState === "edit" && "Edit Quotation"}
                {viewState === "preview" && "Quotation Print Preview"}
              </h2>
              <p className="text-muted small m-0 mt-1">
                {viewState === "list" && "Manage client billing quotations"}
          ...
          {viewState === "list" ? (
                  ...
                  <table className="table table-hover align-middle mb-0">
                    <thead>
                      <tr>
                        ...
                        <th>Total Amount</th>
                        <th>Status</th>
                        <th className="text-center" style={{ width: "360px" }}>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {quotations.map((quot) => (
                        <tr key={quot._id}>
                          <td><strong className="text-primary font-monospace">{quot.invoiceNumber}</strong></td>
                          <td>
                            <div>
                              <div className="fw-bold text-dark">{quot.clientName}</div>
                              {quot.clientGst && <div className="text-muted small font-monospace">GST: {quot.clientGst}</div>}
                            </div>
                          </td>
                          <td className="font-monospace text-secondary">{formatDateShort(quot.invoiceDate)}</td>
                          <td className="text-secondary">{quot.eventDate || "-"}</td>
                          <td><span className="fw-extrabold text-dark font-monospace">₹{formatINR(quot.totalAmount)}</span></td>
                          <td><span className={getStatusBadgeClass(quot.status)}>{quot.status}</span></td>
                          <td className="text-center">
                            <button className="btn btn-sm btn-info text-white me-2 px-2 fw-bold" onClick={() => handleOpenPreview(quot)}>Preview</button>
                            <button className="btn btn-sm btn-outline-secondary me-2 px-2" onClick={() => handleConvertToProformaInvoice(quot)}>→ Proforma</button>
                            <button className="btn btn-sm btn-outline-primary me-2 px-2" onClick={() => handleConvertToRealInvoice(quot)}>→ Invoice</button>
                            <button className="btn btn-sm btn-light me-2 px-2" onClick={() => handleOpenEditForm(quot)}>Edit</button>
                            <button className="btn btn-sm btn-danger px-2" onClick={() => handleDeleteQuotation(quot._id)}>Delete</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="card shadow-sm border-0 fade-in">
              <div className="card-header card-gradient-blue p-4 border-0">
                <h5 className="mb-0 fw-bold">Quotation Details</h5>
              </div>
              <div className="card-body p-4">
                <form onSubmit={handleSubmit}>
                  <div className="row g-3 mb-4">
                    <div className="col-12 col-md-3">
                      <label className="form-label font-monospace text-secondary small fw-bold">Quotation Number</label>
                      <input type="text" className="form-control font-monospace fw-bold" required value={quotationForm.invoiceNumber} onChange={(e) => setQuotationForm({ ...quotationForm, invoiceNumber: e.target.value })} />
                    </div>
                    <div className="col-12 col-md-3">
                      <label className="form-label font-monospace text-secondary small fw-bold">Date</label>
                      <input type="date" className="form-control" required value={quotationForm.invoiceDate} onChange={(e) => setQuotationForm({ ...quotationForm, invoiceDate: e.target.value })} />
                    </div>
                    <div className="col-12 col-md-3">
                      <label className="form-label font-monospace text-secondary small fw-bold">Event Date(s)</label>
                      <input type="text" className="form-control" placeholder="e.g. 22,24 and 25april" value={quotationForm.eventDate} onChange={(e) => setQuotationForm({ ...quotationForm, eventDate: e.target.value })} />
                    </div>
                    <div className="col-12 col-md-3">
                      <label className="form-label font-monospace text-secondary small fw-bold">Status</label>
                      <select className="form-select" value={quotationForm.status} onChange={(e) => setQuotationForm({ ...quotationForm, status: e.target.value })}>
                      ...
                      <tbody>
                        {quotationForm.items.map((item, idx) => (
                          <tr key={idx}>
                            ...
                            <td className="text-center">
                              <button type="button" className="btn btn-sm btn-outline-danger" disabled={quotationForm.items.length === 1} onClick={() => handleRemoveItemRow(idx)}>🗑️</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <div className="col-12 col-md-6">
                      <div className="row g-3">
                        <div className="col-6">
                          ...
                          <input type="number" className="form-control font-monospace" value={quotationForm.cgstRate} onChange={(e) => handleFinancialFieldChange("cgstRate", e.target.value)} />
                        </div>
                        <div className="col-6">
                          <label className="form-label font-monospace text-secondary small fw-bold">SGST Rate (%)</label>
                          <input type="number" className="form-control font-monospace" value={quotationForm.sgstRate} onChange={(e) => handleFinancialFieldChange("sgstRate", e.target.value)} />
                        </div>
                      </div>
                    </div>
                    <div className="col-12 col-md-6">
                      <div className="card bg-light border-0 p-3 shadow-sm text-end">
                        <div className="d-flex justify-content-between mb-2"><span>Subtotal:</span><span className="fw-bold font-monospace">₹{formatINR(quotationForm.subtotal)}</span></div>
                        <div className="d-flex justify-content-between mb-2"><span>CGST ({quotationForm.cgstRate}%):</span><span className="fw-bold font-monospace">₹{formatINR(quotationForm.cgstAmount)}</span></div>
                        <div className="d-flex justify-content-between mb-2"><span>SGST ({quotationForm.sgstRate}%):</span><span className="fw-bold font-monospace">₹{formatINR(quotationForm.sgstAmount)}</span></div>
                        <hr className="my-2" />
                        <div className="d-flex justify-content-between mb-2"><span className="fw-bold text-primary">GRAND TOTAL:</span><span className="fw-extrabold font-monospace text-primary fs-5">₹{formatINR(quotationForm.totalAmount)}</span></div>
                      </div>
                    </div>
                  </div>
                  <div className="row g-4">
                    <div className="col-12 col-md-6">
                      ...
                      <input type="text" className="form-control fw-bold font-monospace" value={quotationForm.totalAmountWords} onChange={(e) => setQuotationForm({ ...quotationForm, totalAmountWords: e.target.value })} />
                    </div>
                    <div className="col-12 col-md-6">
                      <label className="form-label font-monospace text-secondary small fw-bold">Remarks (Optional)</label>
                      <textarea className="form-control" rows="2" value={quotationForm.notes} onChange={(e) => setQuotationForm({ ...quotationForm, notes: e.target.value })} />
                    </div>
                    <div className="col-12">
                      <label className="form-label font-monospace text-secondary small fw-bold">Terms &amp; Conditions</label>
                      <textarea className="form-control font-monospace small" rows="5" value={quotationForm.terms} onChange={(e) => setQuotationForm({ ...quotationForm, terms: e.target.value })} />
                    </div>
                  </div>
                  <div className="d-flex justify-content-end gap-3 mt-4 pt-3 border-top">
                    <button type="button" className="btn btn-outline-secondary px-4" onClick={() => setViewState("list")}>Cancel</button>
                    <button type="submit" className="btn btn-primary px-5">{viewState === "add" ? "Create Quotation" : "Save Changes"}</button>
                  </div>
                </form>
              </div>
            </div>
            ...
            <div className="d-flex flex-column align-items-center bg-dark bg-opacity-10 py-4 overflow-auto rounded-3">
              <div className="invoice-paper-preview invoice-page mb-5 shadow" id="invoice-page-1">
                <div className="text-center mb-4">
                  <h1 className="fw-extrabold text-uppercase m-0" style={{ fontSize: "2.1rem", letterSpacing: "3px" }}>Quotation</h1>
                </div>
                  ...
                  <div className="col-5 text-end">
                    <div className="d-flex flex-column gap-2" style={{ fontSize: "0.85rem" }}>
                      <div className="d-flex justify-content-end align-items-center">
                        <span className="text-secondary me-2 fw-bold" style={{ fontSize: "1rem" }}>Quotation No :</span>
                        <strong style={{ fontSize: "1.2rem", fontFamily: "monospace" }}>{quotationForm.invoiceNumber}</strong>
                      </div>
                      <div><span className="text-secondary me-2">Date :</span><span>{formatDateShort(quotationForm.invoiceDate)}</span></div>
                      <div><span className="text-secondary me-2">Event Date :</span><span>{quotationForm.eventDate || "-"}</span></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="row g-3" style={{ fontSize: "0.85rem" }}>
                  <div className="col-7">
                    ...
                    <div className="text-secondary" style={{ lineHeight: "1.4" }}>
                      ...
                      <strong>Account Name :</strong> {quotationForm.bankAccountName}<br />
                      <strong>Account No :</strong> {quotationForm.bankAccountNumber}<br />
                      <strong>IFSC Code :</strong> {quotationForm.bankIfsc}<br />
                      <strong>Bank Name :</strong> {quotationForm.bankName}
                    </div>
                  </div>
                  <div className="col-5">
                    <table className="w-100 align-middle" style={{ borderCollapse: "collapse", lineHeight: "1.6" }}>
                      <tbody>
                        <tr><td>Subtotal</td><td className="text-end font-monospace">₹ {formatINR(quotationForm.subtotal)}</td></tr>
                        <tr><td>CGST ({quotationForm.cgstRate}%)</td><td className="text-end font-monospace">₹ {formatINR(quotationForm.cgstAmount)}</td></tr>
                        <tr style={{ borderBottom: "1px solid #aaa" }}><td className="pb-1">SGST ({quotationForm.sgstRate}%)</td><td className="text-end font-monospace pb-1">₹ {formatINR(quotationForm.sgstAmount)}</td></tr>
                        <tr className="fw-extrabold" style={{ fontSize: "0.95rem" }}><td className="pt-2">Total</td><td className="text-end font-monospace pt-2">₹ {formatINR(quotationForm.totalAmount)}</td></tr>
                      </tbody>
                    </table>
                  </div>
                </div>
                <div className="mt-4" style={{ fontSize: "0.85rem" }}>
                  ...
                  <span className="fw-bold mt-1 d-block text-secondary">{quotationForm.totalAmountWords}</span>
                </div>
              </div>
              ...
              <div className="invoice-paper-preview invoice-page shadow" id="invoice-page-2">
                <div style={{ minHeight: "80%" }}>
                  <div className="mb-4" style={{ fontSize: "0.75rem", color: "#444", textAlign: "justify" }}>
                    ...
                    <div style={{ whiteSpace: "pre-wrap", lineHeight: "1.3" }}>{quotationForm.terms}</div>
                  </div>
